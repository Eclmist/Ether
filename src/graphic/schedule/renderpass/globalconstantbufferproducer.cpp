...
#include "globalconstantbufferproducer.h"

ETH_NAMESPACE_BEGIN

DEFINE_GFX_PASS(GlobalConstantBufferProducer);

DEFINE_GFX_RESOURCE(GlobalCommonConstants);
DEFINE_GFX_CBV(GlobalCommonConstants);

GlobalConstantBufferProducer::GlobalConstantBufferProducer()
    : RenderPass("Global Constant Buffer Producer")
{
}

void GlobalConstantBufferProducer::Initialize()
{
}
...
void GlobalConstantBufferProducer::Render(GraphicContext& context, ResourceContext& rc)
{
