...
VS_OUTPUT VS_Main(VS_INPUT IN, uint ID: SV_InstanceID)
{
    ...
    float4 positionOffset = (float4(ID % 10, (ID % 100) / 10, ID / 100, 0) - 4.5) * 2.8;
    //positionOffset -= float4(offset * ((ID % 4) + 1), offset * ((ID % 5) + 1), offset * ((ID%6) + 1) * 0.7, 0) * 0.03;
    ...
    {
        float s = 100;
        o.Position = mul(CB_ModelViewProj.MVP, mul(float4x4(s,0,0,0, 0,s,0,0, 0,0,s,0, 0,0,0,1), float4(IN.Position, 1.0)));
        //o.Position.z = -1;
    }
